...
##? X <- a + b:
###* PC <- PC + 1; FETCH; GOTO 36
firmware[35] = 0b000100100_000_00110101_001000_001_001
###* MAR <- MBR; read_word; GOTO 37
firmware[36] = 0b000100101_000_00010100_100000_010_010
###* H <- MDR; GOTO 38
firmware[37] = 0b000100110_000_00010100_000001_000_000
###* PC <- PC + 1; FETCH; GOTO 39
firmware[38] = 0b000100111_000_00110101_001000_001_001
###* MAR <- MBR; read_word; GOTO 40
firmware[39] = 0b000101000_000_00010100_100000_010_010
###* X <- H + MDR, GOTO 0
firmware[40] = 0b000000000_000_00111100_000100_000_000

##? X <- a - b:
###* PC <- PC + 1; FETCH; GOTO 42
firmware[41] = 0b000101010_000_00110101_001000_001_001
###* MAR <- MBR; read_word; GOTO 43
firmware[42] = 0b000101011_000_00010100_100000_010_010
###* H <- MDR; GOTO 44
firmware[43] = 0b000101100_000_00010100_000001_000_000
###* PC <- PC + 1; FETCH; GOTO 45
firmware[44] = 0b000101101_000_00110101_001000_001_001
###* MAR <- MBR; read_word; GOTO 46
firmware[45] = 0b000101110_000_00010100_100000_010_010
###* X <- H + MDR, GOTO 0
firmware[46] = 0b000000000_000_00101010_000100_000_000
...
##? X <- X - 1; GOTO 0
firmware[31] = 0b000000000_000_00110110_000100_000_011

##? X <- X + 1; GOTO 0
firmware[32] = 0b000000000_000_00110101_000100_000_011
...
##? Y <- a + b:
###* PC <- PC + 1; FETCH; GOTO 48
firmware[47] = 0b000110000_000_00110101_001000_001_001
###* MAR <- MBR; read_word; GOTO 49
firmware[48] = 0b000110001_000_00010100_100000_010_010
###* H <- MDR; GOTO 50
firmware[49] = 0b000110010_000_00010100_000001_000_000
###* PC <- PC + 1; FETCH; GOTO 51
firmware[50] = 0b000110011_000_00110101_001000_001_001
###* MAR <- MBR; read_word; GOTO 52
firmware[51] = 0b000110100_000_00010100_100000_010_010
###* Y <- H + MDR, GOTO 0
firmware[52] = 0b000000000_000_00111100_000010_000_000

##? Y <- a - b:
###* PC <- PC + 1; FETCH; GOTO 54
firmware[53] = 0b000110110_000_00110101_001000_001_001
###* MAR <- MBR; read_word; GOTO 55
firmware[54] = 0b000110111_000_00010100_100000_010_010
###* H <- MDR; GOTO 56
firmware[55] = 0b000111000_000_00010100_000001_000_000
###* PC <- PC + 1; FETCH; GOTO 57
firmware[56] = 0b000111001_000_00110101_001000_001_001
###* MAR <- MBR; read_word; GOTO 58
firmware[57] = 0b000111010_000_00010100_100000_010_010
###* Y <- H + MDR, GOTO 0
firmware[58] = 0b000000000_000_00101010_000010_000_000

##? MEMORY[ADDRESS] = Y:
###* PC < PC + 1; FETCH; GOTO 26
firmware[25] = 0b000011010_000_00110101_001000_001_001
###* MAR <- MBR; GOTO 27
firmware[26] = 0b000011011_000_00010100_100000_000_010
...
##? y <- y - 1; GOTO 0
firmware[33] = 0b000000000_000_00110110_000010_000_100

##? y <- y + 1; GOTO 0
firmware[34] = 0b000000000_000_00110101_000010_000_100
